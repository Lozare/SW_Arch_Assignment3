...
def Pay(user):
    ...
    for key in mycart.items:
        dbCursor.execute("Insert Into orders  (orderNum, username, item, number, paymentMethod, shippingAddress, purchaseDate, shippingDate) values('" + str(x) + "','"+str(user)+"','" + str(key) + "','" + str(mycart.items[key]) +"','" + str(card) + "','" + str(address) + "','" + str(d.strftime("%x")) + "','" + str(d.strftime("%x")) + "');")
        dbConnection.commit()
